...
def build_steps() -> list[StepSpec]:
    return [
        ...
        StepSpec(name="upload", script="upload_r2.py", optional=True),
...
def main() -> int:
    ...
    image_base_url = os.environ.get("CF_R2_PUBLIC_URL", "").strip().rstrip("/")
    ...
    for step in steps:
        ...
        if step.name == "upload" and not os.environ.get("CF_R2_PUBLIC_URL"):
            code, message = 0, "skipped upload (CF_R2_PUBLIC_URL not set)"
        elif step.per_video:
            ...
            if code == 0 and step.name == "upload":
                # Capture the public base URL from upload output
                try:
                    upload_result = json.loads(message)
                    if isinstance(upload_result, dict) and upload_result.get("baseUrl"):
                        image_base_url = upload_result["baseUrl"]
                        LOGGER.info("Image base URL from upload: %s", image_base_url)
                except (json.JSONDecodeError, TypeError):
                    pass  # message wasn't JSON, keep env-based URL
